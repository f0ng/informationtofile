    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        ...
        if toolFlag == self._callbacks.TOOL_PROXY or toolFlag == self._callbacks.TOOL_REPEATER or toolFlag == self._callbacks.TOOL_INTRUDER:
            ...
            if not messageIsRequest:
                ...
                assets1 = lytoken(content)
                assets2 = lydingdan(content)
                ...
                if assets != False:
                    ...
                    with open("/Users/f0ngf0ng/BURP/BurpUnlimited/ip/" + filename,"a+") as file:
                ...
                if assets1 != False:
                    ...
                    with open("/test.txt" ,"r+") as file1:
                        ip_lists1 = assets1.split(",")

                        for file_single in file1:
                            b_set.add(file_single.strip())
                        
                        for ip_split in ip_lists1:
                            b_set.add(ip_split.strip())

                    with open("/test1.txt" ,"w+") as file11:
                        for ss in b_set:
                            file11.writelines(ss + '\n')  

                if assets2 != False:

                    with open("/test2.txt" ,"r+") as file2:
                        ip_lists2 = assets2.split(",")

                        for file_single in file2:
                            c_set.add(file_single.strip())

                        for ip_split in ip_lists2:
                            c_set.add(ip_split.strip())
                    
                    with open("/test3" ,"w+") as file22:
                        for ss in c_set:
                            file22.writelines(ss + '\n')  
...
def lytoken(string):
        assets = re.findall(r'[0-9A-Za-z]{32}', string)
        if assets != []:
            assetss = set(assets)
            assetsSet = set()
            for i in assets:
...
def lydingdan(string):
        assets = re.findall(r'[0-9A-Za-z]{17}', string)
        if assets != []:
            assetss = set(assets)
            assetsSet = set()
            for i in assets:
